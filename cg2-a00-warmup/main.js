...
var makePath = function(seperator) {
    ...
    var skateboard = {axes: 2, wheels: 4, board: 1}; // important consideration
    var String = function(a) {
        ...
        this.toString = function() {
            var s = "";
            for(var i = s ? 1 : 0; i < this.str.length - this.sep.length; i++) {
                s += this.str[i];
            }
            return s;
        }
    ...
    var list = new String(seperator);
    return function(a) { // jk, ends with j,k
        if(!a) {
            return list;
        } else {
            list.push(a);
